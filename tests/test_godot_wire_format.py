"""Stage-2 wire format conformance.

The Godot arena (shared/godot_arena/) does not have a protobuf library
in pure GDScript. Rather than vendor one, the arena emits *JSON whose
field names match proto3 exactly*; the candidate-side parses each
message via `google.protobuf.json_format.ParseDict` and gets a typed
proto back.

These tests build the same dicts that arena_main.gd produces — the
shapes are duplicated here verbatim — and assert the round-trip
through `google.protobuf.json_format` yields well-formed protobuf
messages with the expected values. If anyone changes the GDScript
shapes, the matching test below fails first.
"""
...
def _build_arena_main_bundle(*, oracle: bool, frame_id: int = 0) -> dict:
    """Mirror of arena_main.gd::_build_sensor_bundle.

    If this helper drifts from the GDScript source, the JSON the arena
    emits will not parse back to a SensorBundle. Treat any drift as a
    test-breakage with the GDScript side as the source of truth.
    """
    bundle = {
        "frame": {
            "frame_id": frame_id,
...
def test_sensor_bundle_without_oracle_parses() -> None:
    payload = _build_arena_main_bundle(oracle=False, frame_id=0)
    ...
    # Oracle is unset; protobuf3 reports HasField only for messages.
    assert not bundle.HasField("oracle")


def test_sensor_bundle_with_oracle_parses() -> None:
    payload = _build_arena_main_bundle(oracle=True, frame_id=1)
    bundle = json_format.ParseDict(payload, sensor_pb2.SensorBundle())
...
def test_initial_state_parses() -> None:
    payload = {
        "bundle": _build_arena_main_bundle(oracle=False, frame_id=0),
        "zmq_frame_endpoint": "tcp://127.0.0.1:7655",
        "simulator_build_sha256": "stage2-arena-poc-0.6",
    }
    msg = json_format.ParseDict(payload, aiming_pb2.InitialState())
    assert msg.zmq_frame_endpoint == "tcp://127.0.0.1:7655"
    assert msg.simulator_build_sha256 == "stage2-arena-poc-0.6"
...
def test_episode_stats_with_events_parses() -> None:
    payload = {
        "episode_id": "ep-000000000000002a",
        "seed": 42,
        "duration_ns": 90_000_000_000,
...
def test_length_prefix_round_trip() -> None:
    ...
    n = len(body)
    encoded_be = bytes([(n >> 24) & 0xFF, (n >> 16) & 0xFF, (n >> 8) & 0xFF, n & 0xFF])
    via_struct = struct.pack(">I", n)
    assert encoded_be == via_struct
