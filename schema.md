...
> Plan version 0.2 — drafted 2026-04-29; team decisions on the v0.1 open questions folded in (see §10).
...
> Companion repository: [`RM_Vision_Aiming/`](../RM_Vision_Aiming) (the production self-aiming pipeline this assignment is derived from).

---

## 0. TL;DR — what we propose

A **single-scenario, multi-stage coding gauntlet** called **"Aiming Arena"**: a 3D first-person adversarial shooter in which the candidate's C++ aiming stack faces off against a frozen population of RL-trained "ghost" robots inside a custom simulator. The candidate progresses through **six required sub-projects (HW1–HW6) plus one bonus (HW7)** — each a separate ROS2-style C++ package or Python module, each with prebuilt scaffolding and `TODO` slots — culminating in an integrated agent that gets ranked on an **internal-only** leaderboard via GitHub Classroom + a self-hosted CI runner. HW7 is genuinely optional: skipping it does not lower the core ranking (§10 decision 2).
...
| **Submission & grading** | **GitHub Classroom** + **self-hosted GPU runner** running a graded Docker image | Free, scales to ~50 candidates, CSV export drives a tiny Streamlit leaderboard. |
| **Internal leaderboard** | Static page generated hourly by GitHub Actions, served from a private repo behind GitHub team SSO; candidates see only their own row + an anonymized rank/percentile | **Internal only** per §10 decision 3 — public ranking was rejected as a deterrent on applicants. Avoids EvalAI's new paid tier and CodaLab's sunset; Codabench is held in reserve as a future hosted alternative if we ever go public. |
...
Bots receive the same observations a human would (pixel feed + IMU + chassis odometry), so the candidate's stack is competing on equal sensory footing. Their policies are frozen `.pt` files committed to the repo; they are not retrained between submissions, so the leaderboard is reproducible.
...
1. Candidate accepts the GitHub Classroom invite per HW (or one mono-classroom per the recruitment cycle).
2. They develop locally, push to their fork. Each push triggers an autograding workflow on the **self-hosted GPU runner** (label `self-hosted, gpu, aiming-hw`).
3. The workflow:
   1. Spins up the grader image.
   2. Builds the candidate's binary.
   3. Runs unit tests (public, visible to candidate).
   4. Runs grading episodes against the appropriate frozen bot tier.
   5. Writes a JSON score + a 30 s replay MP4 + flame-graph profile.
   6. Posts a PR comment with the score.
4. The candidate sees the score immediately. The hidden grader configuration prevents tampering — per the LUMS scaling autograding case study, GitHub Classroom flags any unauthorized changes to `.github/workflows/*.yaml`.

### 7.2 Leaderboard

* **Internal only** per §10 decision 3. Public ranking was rejected as a deterrent on applicants. The recruiting team sees a fully de-anonymized board; candidates see only their own row plus an anonymized rank/percentile in the autograder PR comment.
* GitHub Action scheduled hourly aggregates the JSON scores from each repository, writes `leaderboard.csv` to a private repo.
* A second Action regenerates a static HTML dashboard (Streamlit-export or pure Jinja) and serves it from the private repo behind GitHub team SSO. **No `gh-pages` public deploy.**
...
* Grader runs in a network-isolated container (no outbound internet). Models must be self-contained inside the repo.
* Candidate cannot read the hidden test fixtures; only the grader sees them. Unit tests are deliberately distinct from the grading episode set.
* Submission count rate-limited (5 per hour) to discourage gradient-attacks against the hidden seeds.
...
| GPU contention on the self-hosted runner | Medium | Low | Use [RunsOn](https://runs-on.com/) or AWS spot fleet as overflow capacity. |
...
*End of plan v0.2 (team decisions on v0.1 open questions folded into §10 and downstream sections). Comments, corrections, and disagreements welcome — file as issues against this file in the recruitment-cycle repo.*
